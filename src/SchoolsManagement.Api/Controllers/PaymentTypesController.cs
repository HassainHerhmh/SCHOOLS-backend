...
public class PaymentTypesController : ControllerBase
{
    ...
    public PaymentTypesController(ApplicationDbContext db)
    {
        _db = db;
    }

    [HttpGet]
    public async Task<ActionResult<IEnumerable<PaymentTypeRecord>>> List(CancellationToken cancellationToken)
    {
        var list = await _db.PaymentTypes.AsNoTracking()
    ...
    [HttpPost]
    public async Task<ActionResult<PaymentTypeRecord>> Create(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var entity = new PaymentTypeRecord
        {
            Code = body.Code,
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<PaymentTypeRecord>> Update(
        int id,
        [FromBody] UpsertAccountGroupRequest body,
        CancellationToken cancellationToken)
    {
        var entity = await _db.PaymentTypes.FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
        if (entity is null)
        {
            return NotFound();
        }
